...
<script language="javascript" src="team_check.js"></script><script type="text/javascript" src="js/laydate.js"></script>
...
<div align="center"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td height="30" align="left"> 组队立项申请状态查看 </td>
        <td></td>
      </tr>
  </table>
  <form action="?action=order" method="post" name="myform" target="_self" id="myform" style="margin:0">
    ...
    <table cellpadding="0" cellspacing="1" border="0" width="100%" align="center" class="table_southidc">
      <tr>
        <td class="back_southidc" height="25" align="center">组队立项申请状态查看</td>
      </tr>
      <tr>
        <td><table width="100%" height="47"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#999999">
          <tr align="center" bgcolor="#EEEEEE">
            ...
            <td width="13%" bgcolor="#EEEEEE">项目成员</td>
            <td width="11%" bgcolor="#EEEEEE">实践时间</td>
		...
		if request("isok")="a" then
		 ...
		 if request("order5")="asc" then
		 	if session("power")=3 then
				sql="select * from team where username='"&session("loginusername")&"'  "&wherestr&" order by addtimes "
			else
				sql="select * from team where 1=1  "&wherestr&" order by addtimes "
			end if
		else
		 	if session("power")=3 then
				sql="select * from team where username='"&session("loginusername")&"'  "&wherestr&" order by addtimes  desc"
			else
				sql="select * from team where 1=1  "&wherestr&" order by addtimes desc "
			end if
	  ...
	  %>
          <tr bgcolor="#FFFFFF">
            ...
            <td align="center" bgcolor="#FFFFFF" ><a href="team_detail.asp?id=<%=rs("id")%>&amp;page=<%=request("page")%>"><font color="blue"><%=rs("bnumber")%></font></a></td>
		   ...
		   response.Write("<font color=red>未审核</font>")
                       ...
                       <%if session("power")=2 then%>
         <td align="center" bgcolor="#FFFFFF"><a href="<%=rs("picurl")%>" target="_blank">附件</a>&nbsp;<a href="team_edit.asp?id=<%=rs("id")%>&amp;page=<%=request("page")%>">修改</a> <a href="team_del.asp?id=<%=rs("id")%>" onClick="return checkDel()">删除</a></td>
         <%else%>
		...
		call showpage("team_manage.asp?keyword="&keyword,totalnumber,maxperpage,true,true,"条")
		%></td>
          </tr>
          <%
		else
