...
<script language="javascript" src="certified_check.js"></script><script type="text/javascript" src="js/laydate.js"></script>
...
<div align="center"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td height="30" align="left"> 认证状态查看 </td>
        <td></td>
      </tr>
  </table>
  <form action="?action=order" method="post" name="myform" target="_self" id="myform" style="margin:0">
    ...
    <table cellpadding="0" cellspacing="1" border="0" width="100%" align="center" class="table_southidc">
      <tr>
        <td class="back_southidc" height="25" align="center">认证状态查看</td>
      </tr>
      <tr>
        <td><table width="100%" height="47"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#999999">
          <tr align="center" bgcolor="#EEEEEE">
            <td width="6%" height="24" bgcolor="#EEEEEE">选择</td>
            
            <td width="8%" bgcolor="#EEEEEE">实践名称</td>
            <td width="10%" bgcolor="#EEEEEE">实践编号</td>
            
            <td width="15%" bgcolor="#EEEEEE"><a href="?order5=<%if request("order5")="asc" then%>desc<%else%>asc<%end if%>&page=<%=currentpage%>&starttime=<%=request("starttime")%>&endtime=<%=request("endtime")%>" class="title">发布时间<%if request("order5")="asc" then%><img src="images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%></a></td>
            <td width="12%" bgcolor="#EEEEEE">发布人</td>
           
            <td width="10%" bgcolor="#EEEEEE">审核状态</td> 
            <%if session("power")=2 then%>
            <td width="22%" bgcolor="#EEEEEE">操作</td>
			<%else%>
            <td width="17%" bgcolor="#EEEEEE">查看附件</td>
            <%end if%>
          </tr>
          <%
	 	dim rs2,sql2
		...
		if request("isok")="a" then
		 ...
		 if request("order5")="asc" then
		 	if session("power")=3 then
				sql="select * from certified where username='"&session("loginusername")&"'  "&wherestr&" order by addtimes "
			else
				sql="select * from certified where 1=1  "&wherestr&" order by addtimes "
			end if
		else
		 	if session("power")=3 then
				sql="select * from certified where username='"&session("loginusername")&"'  "&wherestr&" order by addtimes  desc"
			else
				sql="select * from certified where 1=1  "&wherestr&" order by addtimes desc "
			end if
	  ...
	  %>
          <tr bgcolor="#FFFFFF">
            ...
            <td align="center" bgcolor="#FFFFFF" ><a href="certified_detail.asp?id=<%=rs("id")%>&amp;page=<%=request("page")%>"><font color="blue"><%=rs("bnumber")%></font></a></td>
		   ...
		   response.Write("<font color=red>未审核</font>")
                       ...
                       <%if session("power")=2 then%>
         <td align="center" bgcolor="#FFFFFF"><a href="<%=rs("picurl")%>" target="_blank">附件</a>&nbsp;<a href="certified_edit.asp?id=<%=rs("id")%>&amp;page=<%=request("page")%>">修改</a> <a href="certified_del.asp?id=<%=rs("id")%>" onClick="return checkDel()">删除</a></td>
         <%else%>
         <td align="center" bgcolor="#FFFFFF"><a href="<%=rs("picurl")%>" target="_blank">附件</a> </td>
		...
		call showpage("certified_manage.asp?keyword="&keyword,totalnumber,maxperpage,true,true,"条")
		%></td>
          </tr>
          <%
		else
