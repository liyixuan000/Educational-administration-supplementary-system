	         ...
	         if(document.all.id[i].checked==true)
		         {
                     switch(page)
	                 {
		                ...
		                form.action = "dataclass_del.asp?lag="+lag;
						break;
	                    case "del":
		                if(! confirm("确实要删除选中的记录吗?"))return false;
		                form.action = "data_del.asp?lag="+lag;
						break; 
						case "hidden":
						if(! confirm("确实要将选中的记录设为未审核吗?"))return false;
						form.action = "sethidden_online6.asp?action=hidden"+"&lag="+lag;
						break; 
						case "online":
						if(! confirm("确实要将选中的设为审核吗?"))return false;
						form.action = "sethidden_online6.asp?action=online"+"&lag="+lag;
						break;
	                 }
			         form.submit();
				     return true;
