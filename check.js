	         ...
	         if(document.all.id[i].checked==true)
		         {
                     switch(page)
	                 {
		                ...
		                form.action = "Articleclass_del.asp?lag="+lag;
						break;
	                    case "del":
		                if(! confirm("确实要删除选中的记录吗?"))return false;
		                form.action = "Article_del.asp?lag="+lag;
						break; 
	                    case "Son_del":
		                if(! confirm("确实要删除选中的记录吗?"))return false;
		                form.action = "Son_del.asp?lag="+lag;
						break; 
						case "setnew":
						if(! confirm("确实要将选中的资讯设为新品吗?"))return false;
		       			form.action = "set.asp?action=setnew"+"&lag="+lag;
						break;
						case "setunnew":
		       			if(! confirm("确实要取消选中的新品吗?"))return false;
		       		 	form.action = "set.asp?action=setunnew"+"&lag="+lag;
						break; 
						case "Son_setnew":
						if(! confirm("确实要将选中的资讯内容设为新品吗?"))return false;
		       			form.action = "set.asp?action=Son_setnew"+"&lag="+lag;
						break;
						case "Son_setunnew":
		       			if(! confirm("确实要取消选中的新品吗?"))return false;
		       		 	form.action = "set.asp?action=Son_setunnew"+"&lag="+lag;
						break; 
						case "hidden":
						if(! confirm("确实要将选中的记录隐藏吗?"))return false;
						form.action = "sethidden_online.asp?action=hidden"+"&lag="+lag;
						break; 
						case "online":
						if(! confirm("确实要将选中的记录上线吗?"))return false;
