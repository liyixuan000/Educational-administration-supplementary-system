...
sub delClass(class_id)
	dim sqlc0,rsc0
	sqlc0="select class_id,childSum from Article002class where parent_id="&class_id&" "
...
sql="select class_id,parent_id from Article002class where class_id="&class_id&""
set rs=server.CreateObject("adodb.recordset")
rs.open sql,conn,1,3
if not rs.eof then
	parent_id=rs("parent_id")
	conn.execute ("update Article002class set childSum=childSum-1 where class_id="&parent_id&"")
	conn.execute ("delete from Article where class_id="&class_id&"")
	call delClass(class_id)
	rs.delete
	rs.update
