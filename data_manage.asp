...
sub subClass(class_id)
	...
	do while not rs.eof
		...
		response.write "<td bgcolor='#ffffff' align=center><a href=dataclass_add.asp?class_id="&rs("class_id")&"&lag="&request("lag")&">添加子类</a>&nbsp;&nbsp;<a href=activityclass_Edit.asp?class_id="&rs("class_id")&"&parent_id_Old="&rs("parent_id")&">修改</a>&nbsp;&nbsp;<a href='activityclass_del.asp?class_id="&rs("class_id")&"' onclick='return checkDelClass()'>删除</a></td>"
		response.write "</tr>"
...
<script language="javascript" src="data_check.js"></script>
</head>
<body>
<div align="center">
...
<div align="center"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td height="30" align="left"> 资料列表 </td>
        <td></td>
      </tr>
  </table>
  <form action="?action=order" method="post" name="myform" target="_self" id="myform" style="margin:0">
    ...
    <table cellpadding="0" cellspacing="1" border="0" width="100%" align="center" class="table_southidc">
      <tr>
        <td class="back_southidc" height="25" align="center">资料列表</td>
      </tr>
      <tr>
        <td><table width="100%" height="47"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#999999">
          <tr align="center" bgcolor="#EEEEEE">
            <td width="7%" height="24">选择</td>
            <td width="14%">资料名称</td>
            <td width="9%">资料类别</td>
            <td width="10%">资料形式</td>
            <td width="11%">上传时间</td><td width="11%" bgcolor="#EEEEEE">审核状态</td>
            <%if session("power")=2 then%>
            <td width="20%">操作</td>
            <%else%>
            <td width="18%">查看</td>
		...
		if class_id>0 then wherestr = wherestr &" and class_id="&class_id
			if session("power")=3 then
				sql="select * from Article002 where username='"&session("loginusername")&"'  "&wherestr&" order by id desc"
			else
				sql="select * from Article002 where 1=1  "&wherestr&" order by id desc"
			end if
	  ...
	  %>
          <tr bgcolor="#FFFFFF">
            ...
            <td align="center" ><a href="data_detail.asp?id=<%=rs("id")%>&amp;page=<%=request("page")%>"><font color="blue"><%=rs("title_gb")%></font></a></td>
            <td align="center" ><%
	  dim sqlC,class_name,parent_id
	  sqlC="select class_name_"&lan_txt&",parent_id from Article002Class where  class_id="&rs("class_id")&""
	  ...
	  %></td>
            <td align="center" ><%
	  'dim sqlC,class_name,parent_id
	  sqlC="select class_name_"&lan_txt&",parent_id from Article002Class where  class_id="&rs("pclass_id")&""
	  set rsc=conn.execute (sqlC)
	  if not rsC.eof then
	  	class_name=rsc("class_name_"&lan_txt&"")
	  	response.write class_name
	  ...
	  %></td>
            <td align="center"><%=infotime(rs("addtimes"))%></td>           <td align="center" bgcolor="#FFFFFF"><%if rs("isok")=1 then
		   response.Write("<font color=blue>已审核</font>")
		   else
		   response.Write("<font color=red>未审核</font>")
		   end if%></td> 
                        <%if session("power")=2 then%>
            <td align="center"><a href="<%=rs("picurl")%>" target="_blank">附件</a> &nbsp;<a href="data_edit.asp?id=<%=rs("id")%>&amp;page=<%=request("page")%>">修改</a> <a href="data_del.asp?id=<%=rs("id")%>" onClick="return checkDel()">删除</a></td>
            <%else%>
		...
		call showpage("data_manage.asp?keyword="&keyword,totalnumber,maxperpage,true,true,"条")
		%></td>
          </tr>
          <%
		else
    ...
    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td width="83%"><input name="Submit1" type="button" id="Submit13" value="删除选中记录" style="border-width:1px;border-color:#999999;border-style:solid; background-color: #eeeeee" onClick="return checkSelectAll(myform,'del','<%=request("lag")%>')" /><input name="Submit2" type="button" id="Submit2" value="设为未审核" onClick="return checkSelectAll(myform,'hidden','<%=request("lag")%>')">
