...
{
	if(form.picUrl.value.replace(/\s/gi,"") ==""){alert("请选择上传的图片");return false;}
	if(form.picUrl.value.replace(/\s/gi,"") !="")
	{
		...
		if (str != ".gif" && str != ".jpg" && str != ".jpeg" && str != ".png"){alert("只允许上传的图片格式为:gif|jpg|jpeg|png");return false;}
	}
...
<body><div id="picUrl" style="display:<%=picUrl_style%>;">
...
<input name="picUrl" type="file" id="keJian_picUrl2" size="40">
  <input name="Submit22" type="submit" id="Submit222" value="上传">（支持gif|jpg|png三种类型的图片）
</form></div>
<%
if picUrl_style<>"" then response.write "图片上传成功！<a href="&request.ServerVariables("http_referer")&"><font color=blue>重新上传</font></a>"
%>
</body>
</html>
