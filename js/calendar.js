var cal_Width = 160;//定义日历显示的宽度，至少140
...
{
    if(WebCalendar.eventSrc != window.event.srcElement) getObjectById("meizzCalendarLayer").style.display = "none";
}
...
function WebCalendar() //初始化日历的设置
{
    ...
    this.dayShow    = 39;                       //定义页面上要显示的天数,不能小于35,或大于39
...
//    for (var i=yearMin; i<=yearMax; i++) strIframe += "<option value='"+i+"'>"+i+"年</option>";
    ...
    "    <table id=tableDay height='120px' width='"+ cal_Width +"px' border=0 cellspacing=1 cellpadding=0>";
         ...
         " type=button value='关闭' onclick='parent.hiddenCalendar()' title='"+ WebCalendar.regInfo +"'>" + 
         "</td></tr></table>"+
