...
<script language="javascript" src="person_check.js"></script><script type="text/javascript" src="js/laydate.js"></script>
...
<div align="center"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td height="30" align="left"> 个人信息发布列表 </td>
        <td></td>
      </tr>
  </table>
          ...
          <select name="pclass_id" id="pclass_id"  >
            ...
            <%
				'dim rsc,class_id,selected
			 	set rsc=conn.execute ("select class_id,class_name_"&lan_txt&",class_depth from Article002class where parent_id="&del_class_id&"  order by class_order desc,class_id asc")
				do while not rsc.eof
					class_id=rsc("class_id")
...
<select name="isok" onChange="myform.submit()">
    ...
    <table cellpadding="0" cellspacing="1" border="0" width="100%" align="center" class="table_southidc">
      <tr>
        <td class="back_southidc" height="25" align="center">信息列表</td>
      </tr>
      <tr>
        <td><table width="100%" height="47"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#999999">
          <tr align="center" bgcolor="#EEEEEE">
            ...
            <td width="11%" bgcolor="#EEEEEE">资料名称</td>
            <td width="13%" bgcolor="#EEEEEE">资料类别</td>
            <td width="11%" bgcolor="#EEEEEE">学号</td>
		...
		if request("isok")="a" then
		 ...
		 if request("order5")="asc" then
		 	if session("power")=3 then
				sql="select * from person where username='"&session("loginusername")&"'  "&wherestr&" order by addtimes "
			else
				sql="select * from person where 1=1  "&wherestr&" order by addtimes "
			end if
		else
		 	if session("power")=3 then
				sql="select * from person where username='"&session("loginusername")&"'  "&wherestr&" order by addtimes  desc"
			else
				sql="select * from person where 1=1  "&wherestr&" order by addtimes desc "
			end if
	  ...
	  %>
          <tr bgcolor="#FFFFFF">
            ...
            <td align="center" bgcolor="#FFFFFF" ><a href="person_detail.asp?id=<%=rs("id")%>&amp;page=<%=request("page")%>"><font color="blue"><%=rs("title")%></font></a></td>
            <td align="center" bgcolor="#FFFFFF" ><%
	  dim sqlC,class_name,parent_id
	  sqlC="select class_name_"&lan_txt&" from Article002Class where  class_id="&rs("class_id")&""
	  ...
	  %></td>
            <td align="center" bgcolor="#FFFFFF" ><%=rs("bnumber")%></td>
		   ...
		   response.Write("<font color=red>未审核</font>")
                       ...
                       <%if session("power")=3 then%>
         <td align="center" bgcolor="#FFFFFF">&nbsp;<a href="person_edit.asp?id=<%=rs("id")%>&amp;page=<%=request("page")%>">修改</a> <a href="person_del.asp?id=<%=rs("id")%>" onClick="return checkDel()">删除</a></td>
		...
		call showpage("person_manage.asp?keyword="&keyword,totalnumber,maxperpage,true,true,"条")
		%></td>
          </tr>
          <%
		else
