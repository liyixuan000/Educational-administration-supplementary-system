...
Sub EditClass()
	...
	sql="select class_depth,childSum from Articleclass where class_id="&parent_id&""
	...
	sql="select * from Articleclass where class_id="&class_id&""
	set rs=server.createobject("adodb.recordset")
	rs.open sql,conn,1,3
	...
	rs.close
	set rs=nothing
	response.write "<script>alert('修改成功');window.location.href='activityclass_Manage.asp?lag="&request("lag")&"'</script>"
	response.end
End Sub
...
<td bgcolor="#99CCFF"><%dim rsEdit
		set rsEdit = conn.execute ("select * from Articleclass where Class_id="&Class_id&"")
