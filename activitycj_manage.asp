...
<script language="javascript" src="actcj_check.js"></script><script type="text/javascript" src="js/laydate.js"></script>
...
<div align="center"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td height="30" align="left"> 活动认证申请列表 </td>
        <td></td>
      </tr>
  </table>
    ...
    <table width="100%"  border="0" align="center">
      <tr>
        <td align="left"><span class="category">
          <select name="class_id" id="class_id" onChange="processcityrequest(this.options[this.options.selectedIndex].value)" >
            <option value="0">选择类别</option>
            <%
				dim rsc,selected,qqq
				qqq=0
				dim del_class_id
          ...
          <select name="pclass_id" id="pclass_id" onChange="processcityrequest2(this.options[this.options.selectedIndex].value)" >
        ...
        <select name="pid"  id="pid" onChange="myform.submit()">
        <option value="">所有活动</option>


      </select><select name="isok" onChange="myform.submit()">
    ...
    <table cellpadding="0" cellspacing="1" border="0" width="100%" align="center" class="table_southidc">
      ...
      <tr>
        <td><table width="100%" height="47"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#999999">
          <tr align="center" bgcolor="#EEEEEE">
            ...
            <td width="11%">活动编号</td><td width="13%">活动名称</td>
            <td width="11%">所获奖项</td>
            <td width="10%" bgcolor="#EEEEEE"><a href="?order5=<%if request("order5")="asc" then%>desc<%else%>asc<%end if%>&page=<%=currentpage%>&starttime=<%=request("starttime")%>&endtime=<%=request("endtime")%>" class="title">申请时间<%if request("order5")="asc" then%><img src="images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%></a></td>
            <td width="9%" bgcolor="#EEEEEE">申请人</td>
		...
		if request("isok")="a" then
		 ...
		 if request("order5")="asc" then
		 	if session("power")=3 then
				sql="select * from apply where username='"&session("loginusername")&"'  "&wherestr&" order by addtimes "
			else
				sql="select * from apply where 1=1  "&wherestr&" order by addtimes "
			end if
		else
		 	if session("power")=3 then
				sql="select * from apply where username='"&session("loginusername")&"'  "&wherestr&" order by addtimes  desc"
			else
				sql="select * from apply where 1=1  "&wherestr&" order by addtimes desc "
			end if
	  ...
	  %>
          <tr bgcolor="#FFFFFF">
            ...
            <td align="center" ><a href="activitycj_detail.asp?id=<%=rs("id")%>&amp;page=<%=request("page")%>"><font color="blue"><%=rs("bnumber")%></font></a></td>
            <td align="center" ><%
	  dim sqlC,class_name,parent_id
	  sqlC="select title_"&lan_txt&" from Article where  id="&rs("pid")&""
	  ...
	  %></td>
            <td align="center" ><%=rs("prize")%></td>
            <td align="center" bgcolor="#FFFFFF"><%=infotime(rs("addtimes"))%></td>
            <td align="center" bgcolor="#FFFFFF"><%=rs("username")%></td>
           <td align="center"><%if rs("isok")=1 then
		   response.Write("<font color=blue>已审核</font>")
		   else
		   response.Write("<font color=red>未审核</font>")
		   end if%></td> 
                       <%if session("power")=3 then%>
         <td align="center"><a href="<%=rs("picurl")%>" target="_blank">成绩证明</a> </td>
            <%else%>
            <td align="center"><a href="<%=rs("picurl")%>" target="_blank">成绩证明</a> &nbsp;<a href="activitycj_edit.asp?id=<%=rs("id")%>&amp;page=<%=request("page")%>">修改</a> <a href="activitycj_del.asp?id=<%=rs("id")%>" onClick="return checkDel()">删除</a></td>
            <%end if%>
		...
		call showpage("activitycj_manage.asp?keyword="&keyword,totalnumber,maxperpage,true,true,"条")
		%></td>
          </tr>
          <%
		else
