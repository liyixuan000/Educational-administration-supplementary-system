...
sub subClass(class_id)
	...
	do while not rs.eof
		...
		response.write "<td bgcolor='#ffffff' align=center><a href=dataclass_add.asp?class_id="&rs("class_id")&"&lag="&request("lag")&">添加子类</a>&nbsp;&nbsp;<a href=dataclass_Edit.asp?class_id="&rs("class_id")&"&parent_id_Old="&rs("parent_id")&">修改</a>&nbsp;&nbsp;<a href='dataclass_del.asp?class_id="&rs("class_id")&"' onclick='return checkDelClass()'>删除</a></td>"
		response.write "</tr>"
...
<div align="center"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td height="30" align="left"> 资料类别管理 </td>
        <td></td>
      </tr>
  </table>
...
<form name="form1" method="post" action="">
  <table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td><input type="button" name="Submit" value="添加类别" onClick="window.location.href='dataclass_add.asp?lag=<%=request("lag")%>'"></td>
      <td align="right">&nbsp;</td>
    </tr>
  </table>
  <table cellpadding="0" cellspacing="1" border="0" width="95%" align="center" class="table_southidc">
	<tr>
		<td class="back_southidc" height="25" align="center">资料类别管理</td>
	</tr>
	<tr>
	...
	rsCounter0.open "select * from Article002class where parent_id=0",conn,1,1
	counter0=rsCounter0.recordcount
	rsCounter0.close:set rsCounter0=nothing
	  sql = "select * from Article002class where parent_id=0  order by class_order desc,class_id asc"
    ...
    <tr bgcolor="#FFFFFF">
     ...
     <!-- <td align="center"><input name="class_order" type="text" id="class_order" value="<%=rs0("class_order")%>" size="6" maxlength="6">
      ...
      <td align="center" bgcolor="#FFFFFF"><a href=dataclass_add.asp?class_id=<%=rs0("class_id")%>&lag=<%=request("lag")%>>添加子类</a>&nbsp;&nbsp;<a href="dataclass_Edit.asp?class_id=<%=rs0("class_id")%>&parent_id_Old=<%=rs0("parent_id")%>&lag=<%=request("lag")%>">修改</a>&nbsp;&nbsp;<a href="dataclass_del.asp?class_id=<%=rs0("class_id")%>&lag=<%=request("lag")%>" onClick="return checkDelClass()">删除</a></td>
    </tr>
    <%
