...
Sub AddClass()
	...
	sqlstr="select * from Article002class "
	...
	if parent_id=0 then
		...
		dim sql0,rs0
		sql0="select class_depth,childSum from Article002class where class_id="&parent_id&""
	...
	rs.close
	set rs=nothing
	response.write "<script>alert('添加成功');window.location.href='dataclass_Manage.asp?lag="&request("lag")&"'</script>"
	response.end
End Sub
