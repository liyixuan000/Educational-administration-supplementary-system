...
<!--#include file="incc/callname.asp"-->
...
<table width="778" border="0" cellpadding="0" bgcolor="#99CCFF">
  ...
  <tr>
    <td width="205" valign="top">        			<li><a href="activity_manage.asp" target="main">
					<span style="background-color: #C0C0C0">
					<font color="#000080">活动列表</font></span></a> <li><a href="activitycj_add.asp" target="main"><font color="#000080">
					<span style="background-color: #C0C0C0">申请活动成绩认证</span></font></a></li>
					                  <li><a href="activitycj_manage.asp" target="main">
					<font color="#000080">
					<span style="background-color: #C0C0C0">活动认证申请列表</span></font></a></li>
					<li><a href="activity_add.asp" target="main"><font color="#000080">
  ...
  <table width="80%"   border="0" cellpadding="3" cellspacing="1" bgcolor="#B5CDE3">
...
sql1="update Eptime_Activity set [isok]=true,[shenhe]='"&request.Cookies("eptime_username")&"',[shenhetime]='"&now()&"' where id="&request.QueryString("id")
...
<form name="form2">
  <tr> 
	...
	<td width="*" align="right">
	  ...
	  &nbsp;<%
	'if nowtype="" then
	  sql="select * from Eptime_Aqoute order by id"
	'else
	  'sql="select * from Eptime_Aqoute where type="&nowtype&" order by id"
	'end if
	...
	sql="select * from Eptime_Class where id_bigclass="&nowbigclass2&" order by id"
...
sql="select * from Eptime_Activity where id_login="&request.Cookies("eptime_id")&""
Else
sql="select * from Eptime_ACtivity where 1=1"
End if
...
%>
	<div align="center">
<table width="96%" border="0" cellpadding="0" cellspacing="0" bgcolor="#99CCFF">
<tr>
<td></td>
<td>
<!--startprint-->
...
<td class="category">业务项目</td>
<td class="category">奖项</td>
...
<%End if%>
 ...
 ' rs_money.open sql,conn,1,3
  ...
  <tr onMouseOver="this.className='highlight'" onMouseOut="this.className=''" onDblClick="javascript:var win=window.open('addchakan_show.asp?id=<%=rs_money("id")%>','帐务详细信息','width=800,height=580,top=20,left=161,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes'); win.focus()" <%if ii mod 2 = 0 then%> bgcolor="FFFFFF" <%End if%>>
	...
	<td align="left">
	  <%
	  sql="select * from Eptime_Aqoute where id="&rs_money("id_bigclass")
	  set rs_bigclass=conn.execute(sql)
	  %>
	  <%if rs_bigclass.eof=false then%><%=rs_bigclass("bigclass")%><%else%>&nbsp;<%end if%>->
	  <%
	  sql="select * from Eptime_Class where id="&rs_money("id_smallclass")
	  set rs_smallclass=conn.execute(sql)
	  %>
...
<%
sql3="select * from Eptime_Activity where id="&rs_money("id")
